...
import Con_Accordian from "./Con_Accordian";
        ...
        <div>
            {
                dummyData.map((data, index) => {
                    return (
                        openIndex === index ?
                            <Con_Accordian dummyData={data} setOpenIndex={() => { openIndex === index ? setOpenIndex(-1) : setOpenIndex(index) }}  open={true} key={index} /> :
                            <Con_Accordian dummyData={data} setOpenIndex={() => { openIndex === index ? setOpenIndex(-1) : setOpenIndex(index) }} open={false} key={index} />
                    )
                })
            }
        </div>
...
export default Con_ParentOfAccordian;
